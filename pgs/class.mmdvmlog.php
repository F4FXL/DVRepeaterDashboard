...
class MMDVMLog
{
    ...
    public function getHeardList()
    {
        ...
        foreach ($logLines as $logLine) {
            if (!$this->isValidLine($logLine)/* || !strpos($logLine, "12:")*/) {
                continue;
            }
            ...
            if ($parseOk) {
                ...
                unset($heardList[$heardItem->_callsign]);
    ...
    private function parseDStarEOT($logLine, $heardItem)
    {
        ...
        if (count($matches)) {
            ...
            $heardItem->_istxing = false;
            if(count($matches) > 12) $heardItem->_rssi = $matches[12][0];
    ...
    // M: 2022-10-07 13:23:18.869 D-Star, transmission lost from F8DSN   /5100 to        I, 3.9 seconds, BER: 14.6%
